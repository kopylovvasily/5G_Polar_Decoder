function  msgcapSCL = fixedSCLDecoder(r,K,E,nL,nMax, ni, nf)
...
%     maxqr= 31; %max integer received value 
   ...
   % f=@(a,b)(1-2*(a<0)).*(1-2*(b<0)).*min(abs(a),abs(b));%minsum
    ...
    g=@(a,b,c)RealADD(b,RealMULT(a,RealSUB(1, RealMULT(2,c,fixp,qtype),fixp, qtype),fixp,qtype),fixp,qtype);
                ...
                msg_capl=squeeze(ucap(:,n+1, F2(F1==0)));
                cout=1;
                for c1=1:nL
                   mid = Deinterleaver(msg_capl(c1,:),true)';
                   [q1,r1]=gfdeconv(fliplr(mid),crcg);
                   %q1 = RealDIV(fliplr(msg_capl(c1,:)),crcg,fixp,qtype);
                   %r1 = RealSUB(fliplr(msg_capl(c1,:)),RealMULT(q1,crcg,fixp,qtype),fixp,qtype);   
                   r1 = RealRESIZE(r1, fixp, qtype);
                   if isequal(r1,0)
                       cout=c1;
                       break
                   end
