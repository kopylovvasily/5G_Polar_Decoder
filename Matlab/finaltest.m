...
linkDir = 'DL';   
KK = [ 56, 43, 54, 164, 164]; %message length with CRC
EE = [ 864, 180, 124, 240, 184];
...
for i = 1:1:5
    ...
    int = ni(i);
    frac = nf(i);
    fixp = {int,frac, 's'}; % # of integer bits (wo the sign), # of fractional bits, signed
    E = EE(i);
    K = KK(i);
    ii = 1;
    for EbNo = -5:0.5:5
        display(EbNo);
        display(i);
        NblkerrsSCL3 = 0; NblkerrsSC3 = 0;
        NblkerrsSCL2 = 0; NblkerrsSC2 = 0;
        Nblocks = numFrames; NblkerrsSCL1 = 0; NblkerrsSC1 = 0;
        for iii=1:1:Nblocks
...
%             noiseVar = 1./(10.^(snrdB/10));
            ...
            fixeddecIn = RealRESIZE(decIn, fixp, qtype);
            % Polar decode
%             decBits = nrPolarDecode(decIn,K,E,L,nMax,false,crcLen);
%             decBits = Deinterleaver(decBits,true)';
%             decBits = decBits(1:K-crcLen);
            
            %#decBits1 = nrPolarDecode(decIn,K,E,L,nMax,true,crcLen);
            %#decBits1 = decBits1(1:K-crcLen);
             decBits = floatSCLDecoder(decIn',K,E,L,nMax);
             decBits = Deinterleaver(decBits,true);
             decBits = decBits(1:K-crcLen);
            
            ourmsgcapFLOAT = floatSCLDecoder(fixeddecIn',K,E,L,nMax);
            ourmsgcapFLOAT = Deinterleaver(ourmsgcapFLOAT,true);
            ourmsgcapFLOAT = ourmsgcapFLOAT(1:K-crcLen);
        
%             
            ourmsgcapFIXED = fixedSCLDecoder(fixeddecIn',K,E,L,nMax, int, frac);
            ourmsgcapFIXED = Deinterleaver(ourmsgcapFIXED,true);
            ourmsgcapFIXED = ourmsgcapFIXED(1:K-crcLen);
            
           % NerrsSCL = sum(ourmsgcapFLOAT ~=  decBits);
            
            NerrsSCL1 = sum( msg' ~=  decBits);
            NerrsSCL2 = sum( msg ~=  ourmsgcapFLOAT);
            NerrsSCL3 = sum( msg ~=  ourmsgcapFIXED);
...
%             
            ...
            if NerrsSCL3>0
                %NbiterrsSCL1 = NbiterrsSCL1+NerrsSCL1;
                NblkerrsSCL3 = NblkerrsSCL3+1;
                NerrsSCL3 = 0;
            end
          
        end
        FER_simSCL1 = NblkerrsSCL1/Nblocks;
        FER_simSCL2 = NblkerrsSCL2/Nblocks;
        FER_simSCL3 = NblkerrsSCL3/Nblocks;
        NblkerrsSCL1 = 0;
        NblkerrsSCL2 = 0;
        NblkerrsSCL3 = 0;
    ...
    clrd = ["-.r*" "-.g*" "-.k*" "-.b*" "-.c*"];
        ...
        plot(xx, y3,clrd(i), 'LineWidth',1.5)
        hold off
end

     str00 = {strcat('float')};   
     str10 = {strcat('fixed', '[' , num2str(ni(1)), num2str(nf(1)), ']')};
     str20 = {strcat('fixed', '[' , num2str(ni(2)), num2str(nf(2)), ']')};
     str30 = {strcat('fixed', '[' , num2str(ni(3)), num2str(nf(3)), ']')};
     str40 = {strcat('fixed', '[' , num2str(ni(4)), num2str(nf(4)), ']')};
     str50 = {strcat('fixed', '[' , num2str(ni(5)), num2str(nf(5)), ']')};
    ...
    legend(str1{:}, '', '', str2{:}, '', '', str3{:}, '', '', str4{:}, '', '', str5{:}, '', '', 'Location','southwest'); 
    %legend(str00{:}, str10{:}, str20{:}, str30{:}, str40{:}, str50{:}, 'Location','southwest');
    ...
    title([strcat('ListSize:',{' '},num2str(L),{'   '}, 'Rate:', {' '}, num2str(KK(cho)), '/', num2str(EE(cho)))])
    
    %display([ourmsgcapFLOAT,decBits1, msg])
   % display(NerrsSCL1);
    
    
    % ourmsgcrcINT = Interleaver(msgcrc,true);
    % ourmsgcrcINT = ourmsgcrcINT';
    %
    % encOut = PolarEncoder(ourmsgcrcINT',E,nMax,Q);
    % encOut1 = nrPolarEncode(msgcrc,E,nMax,true);
    % NerrsSCL2 = sum(encOut ~= encOut1');
    % display(NerrsSCL2);